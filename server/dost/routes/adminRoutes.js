...
router.get('/stats', async (req, res) => {
    try {
        ...
        const activeVirtualAccounts = await models_1.VirtualAccount.countDocuments({ status: 'active' });
...
router.get('/disputes', isAdmin, async (req, res) => {
    try {
        const disputes = await models_1.SettlementDispute.find().sort({ createdAt: -1 });
        res.json({
            success: true,
            data: disputes
        });
 ...
 */
router.post('/dispute', isAdmin, async (req, res) => {
    try {
        const { settlementReference, reason, amount, priority } = req.body;
        const dispute = await models_1.SettlementDispute.create({
            settlementReference,
            reason,
            amount,
            priority,
 ...
 */
router.patch('/dispute/:id', isAdmin, async (req, res) => {
    try {
        const { status, adminNote } = req.body;
        const dispute = await models_1.SettlementDispute.findByIdAndUpdate(req.params.id, {
            status,
            adminNote,
            ...(status === 'RESOLVED' || status === 'REJECTED' ? { resolvedBy: req.user._id } : {})
        }, { new: true });
        res.json({
            success: true,
            data: dispute,
 ...
 */
router.put('/settlements/global-config', async (req, res) => {
    try {
        ...
        const totalPercentage = settlementAccounts.reduce((sum, acc) => {
 ...
 */
router.get('/settlements/global-config', async (req, res) => {
    try {
...
router.get('/system/cron-status', async (req, res) => {
    try {
        const status = CronService_1.cronService.getStatus();
        res.json({
            success: true,
            data: status
        });
 ...
 */
router.get('/verify-connectivity', async (req, res) => {
    try {
        // Use getBankList as a simple connectivity check
        const response = await PalmPayService_1.palmPayService.getBankList();
        res.json({
            success: true,
