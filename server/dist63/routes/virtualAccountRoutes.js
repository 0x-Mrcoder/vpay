...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const { accountName, reference, firstName, lastName, email, phone, bvn } = req.body;
        // Get user details
        ...
        // Allow Level 1 (Tier 1) and above to create accounts
        if (user.kycLevel < 1) {
            res.status(403).json({
                success: false,
        ...
        // Map frontend identity type to PalmPay V2 constants
        let identityType = req.body.identityType || 'individual';
        if (identityType.toLowerCase() === 'individual') {
        ...
        const payload = {
            ...
            licenseNumber: req.body.licenseNumber || req.body.bvn // Use BVN if licenseNumber is missing
        };
        console.log('Sending payload to PalmPay V2.0:', JSON.stringify(payload, null, 2));
        const accountData = await services_1.palmPayService.createVirtualAccountV2(payload);
        // Save virtual account to database
        const virtualAccount = new models_1.VirtualAccount({
            userId: new mongoose_1.default.Types.ObjectId(userId),
            accountNumber: accountData.virtualAccountNo,
            accountName: accountData.virtualAccountName,
            bankName: 'PalmPay',
            bankType: accountData.identityType,
