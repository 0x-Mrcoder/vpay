...
class PalmPayService {
    ...
    loadPrivateKey() {
        try {
            const keyContent = config_1.default.palmpay.privateKey;
            if (keyContent) {
    ...
    generateSignatureV2(data) {
        try {
            if (!this.privateKey) {
                logger_1.logger.error('PalmPay private key not loaded');
                return '';
            }
            ...
            const strA = Object.keys(filteredParams)
                ...
                .join('&');
            logger_1.logger.debug(`String to MD5: "${strA}"`);
            // 3. MD5 Hash and Uppercase
            const md5Hash = crypto.createHash('md5').update(strA).digest('hex').toUpperCase();
            logger_1.logger.debug(`MD5 Hash (Uppercase): "${md5Hash}"`);
            // 4. Sign the MD5 hash using RSA-SHA1 (SHA1withRSA)
            const sign = crypto.createSign('RSA-SHA1');
            sign.update(md5Hash);
            const signature = sign.sign(this.privateKey, 'base64');
            logger_1.logger.debug(`Generated Signature: ${signature}`);
            return signature;
        }
        catch (error) {
            logger_1.logger.error('V2 Signature generation failed', error);
            return '';
        }
    }
     ...
     */
    async createVirtualAccountV2(data) {
        try {
            const payload = {
            ...
            logger_1.logger.info('Creating PalmPay Virtual Account V2.0', { ...payload, licenseNumber: '***' });
            const response = await this.client.post('/virtual/account/label/create', payload);
            ...
            logger_1.logger.error('Failed to create virtual account V2.0', error);
            throw error;
        }
    }
    /**
    ...
    async initiateTransfer(data) {
        try {
            ...
            logger_1.logger.error(`[PalmPay] Transfer initiation failed: ${JSON.stringify(errorMsg)}`);
            throw error;
        }
    }
    /**
    ...
    async getBankList() {
        try {
            ...
            if (Array.isArray(dataObj)) {
                return dataObj.map((b) => ({ code: b.bankCode, name: b.bankName }));
            }
            else if (dataObj) {
                // If it's a single object
                return [{ code: dataObj.bankCode, name: dataObj.bankName }];
            }
            return this.getFallbackBankList();
        }
        catch (error) {
