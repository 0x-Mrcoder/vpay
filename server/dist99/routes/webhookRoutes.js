...
// ─────────────────────────────────────────────────────────────────────────────
router.post('/palmpay/payout', async (req, res) => {
    try {
        const body = req.body;
        logger_1.logger.info('[PAYOUT WEBHOOK] ══════ PalmPay Payout Webhook Received ══════');
        logger_1.logger.info('[PAYOUT WEBHOOK] Body: ' + JSON.stringify(body, null, 2));
        ...
        if (!orderNo) {
            logger_1.logger.warn('[PAYOUT WEBHOOK] ⚠️ Missing orderNo in payload');
            res.status(200).send('success');
            return;
        }
        const payout = await Payout.findOne({ reference: orderNo });
        if (!payout) {
            logger_1.logger.warn(`[PAYOUT WEBHOOK] ⚠️ Payout not found for orderNo: ${orderNo}`);
            res.status(200).send('success');
            return;
        }
        ...
        if (state === 2) {
            ...
            logger_1.logger.info(`[PAYOUT WEBHOOK] ✅ Payout ${orderNo} marked as success.`);
        }
        else if (state === 3 || state === 4 || state === -1) {
            await payoutService.handlePayoutFailure(payout, body.errorMsg || 'Transaction failed');
            logger_1.logger.info(`[PAYOUT WEBHOOK] ❌ Payout ${orderNo} marked as failed. Reason: ${body.errorMsg}`);
        }
