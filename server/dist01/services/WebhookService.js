...
class WebhookService {
    ...
    async handleDeposit(data) {
        ...
        let clearanceDate = new Date(Date.now() + 24 * 60 * 60 * 1000 + 5 * 60 * 1000);
        let limitExceeded = false;
        let limitErrorMessage = '';
        try {
            const { limitService } = await Promise.resolve().then(() => __importStar(require('./LimitService')));
            await limitService.checkTierLimits(virtualAccount.userId.toString(), 'deposit', amount);
        }
        catch (error) {
            limitExceeded = true;
            limitErrorMessage = error.message;
            logger_1.logger.warn(`[DEPOSIT] Limit exceeded for user ${virtualAccount.userId}: ${limitErrorMessage}`);
            // If limit exceeded, we still take the money but maybe delay clearance indefinitely or mark for review
            // For now, let's just flag it in metadata.
        }
        ...
        try {
            const transaction = await WalletService_1.walletService.creditWallet(virtualAccount.userId.toString(), netAmount, 'deposit', `Deposit from ${payerName || 'Unknown'} (${payerAccount || '****'})`, orderNo, {
                ...
                appliedFee: fee,
                limitExceeded,
                limitErrorMessage: limitExceeded ? limitErrorMessage : undefined
