...
export class WebhookService {
    ...
    constructor() {
        ...
        if (this.webhookSecret === 'default-webhook-secret') {
            logger.warn('⚠️  Webhook secret not configured (using insecure default)!');
        }
    }
    ...
    verifySignature(payload: string, rawSignature: string): boolean {
        if (!rawSignature || !payload) {
            logger.error('[SIG] Missing signature or payload for verification');
            return false;
        }
        ...
        try {
            ...
            if (!publicKey) {
                logger.error('[SIG] PalmPay public key not configured');
                return false;
            }
            ...
            if (verify256.verify(formattedKey, signature, 'base64')) {
                logger.info('[SIG] ✅ Verified via RSA-SHA256');
                return true;
            }
            ...
            if (verify1.verify(formattedKey, signature, 'base64')) {
                logger.info('[SIG] ✅ Verified via RSA-SHA1');
                return true;
            }
            ...
            const md5Hash = crypto
                ...
                .digest('hex')
                .toUpperCase();
            const verifyMD5 = crypto.createVerify('RSA-SHA1');
            verifyMD5.update(md5Hash);
            if (verifyMD5.verify(formattedKey, signature, 'base64')) {
                logger.info('[SIG] ✅ Verified via RSA-SHA1 + MD5(payload)');
                return true;
            }

            logger.error('[SIG] ❌ All verification attempts failed (RSA-SHA256, RSA-SHA1, RSA-MD5-SHA1)');
            return false;
        } catch (error) {
            logger.error('[SIG] Verification threw an exception', error);
            return false;
        }
    }
    ...
    verifyBodySignature(body: any, rawSignature: string): boolean {
        try {
            ...
            logger.info(`[SIG] Constructed body-signature payload: ${payload}`);
            return this.verifySignature(payload, signature);
        } catch (e) {
            logger.error('[SIG] verifyBodySignature error', e);
            return false;
        }
    }
    ...
    isTimestampValid(createdTime: number | string | undefined): boolean {
        if (!createdTime) {
            ...
            logger.warn('[TIMESTAMP] No createdTime in webhook body, skipping timestamp check');
            return true;
        }
        ...
        if (agMs > WEBHOOK_TIMESTAMP_WINDOW_MS) {
            logger.warn(
                `[TIMESTAMP] ❌ Webhook is ${Math.round(agMs / 1000)}s old (max ${WEBHOOK_TIMESTAMP_WINDOW_MS / 1000}s). Rejecting.`
            );
            return false;
        }
        return true;
     ...
     */
    async isReplayWebhook(orderNo: string): Promise<boolean> {
        if (!orderNo) return false;
        const existing = await WebhookLog.findOne({ orderNo });
        if (existing) {
            logger.warn(`[REPLAY] ⚠️  Duplicate webhook received for orderNo: ${orderNo}`);
            return true;
        }
        return false;
    }
    ...
    async logWebhook(
        ...
        try {
            ...
            if (error.code === 11000) {
                logger.warn(`[LOG] Duplicate webhook log for orderNo: ${orderNo}`);
                return null;
            }
            logger.error('[LOG] Failed to log webhook', error);
            return null;
        }
    }
    ...
    async updateWebhookLog(
        logId: string,
        updateData: { dispatchStatus: string; userId?: string; processingResult?: string }
    ): Promise<void> {
        try {
            await WebhookLog.findByIdAndUpdate(logId, updateData);
        } catch (error) {
            logger.error('[LOG] Failed to update webhook log', error);
        }
    }
    ...
    async processWebhook(event: any): Promise<{ success: boolean; message: string; userId?: string }> {
        try {
            ...
            logger.info(`[PROCESS] Handling webhook event type: ${type}`);
            ...
            logger.warn(`[PROCESS] Unhandled event type: ${type}`);
            return { success: true, message: 'Event ignored' };
        } catch (error: any) {
            logger.error('[PROCESS] Webhook processing failed', error);
    ...
    private async handleDeposit(
        ...
        const externalReference =
            data.externalReference || data.orderId || data.paymentReference;

        logger.info(
            `[DEPOSIT] OrderNo=${orderNo}, Amount=${amount}, Status=${status}, VA=${virtualAccountNo}`
        );

        // Normalise status
        const isSuccess =
            status === 'SUCCESS' || status === '00' || status === 1 || status === '1';

        if (!isSuccess) {
            logger.info(`[DEPOSIT] Status is ${status}, ignoring.`);
        ...
        if (!virtualAccount) {
            logger.error(`[DEPOSIT] Virtual Account not found: ${virtualAccountNo}`);
        ...
        if (existingTxn) {
            logger.info(`[DEPOSIT] ✅ Transaction ${orderNo} already processed. Skipping.`);
            return {
                success: true,
     ...
     */
    async reconcileMissedDeposits(): Promise<void> {
        logger.info('[RECONCILE] 🔄 Running PalmPay deposit reconciliation job...');
        ...
        try {
            ...
            if (pendingDeposits.length === 0) {
                logger.info('[RECONCILE] No pending PalmPay deposits to reconcile.');
                return;
            }

            logger.info(`[RECONCILE] Found ${pendingDeposits.length} pending deposits to check.`);

            const palmPayBaseUrl =
                process.env.PALMPAY_BASE_URL || 'https://sandbox.palmpay.com/v2';
            ...
            for (const deposit of pendingDeposits) {
                ...
                try {
                    const response = await axios.get(
                        `${palmPayBaseUrl}/virtual/account/query`,
                        {
                            params: { orderNo },
                            headers: {
                    ...
                    if (
                        ...
                        logger.info(
                            `[RECONCILE] ✅ Reconciled deposit ${orderNo} → marked as success`
                        );
                    } else {
                        logger.info(
                            `[RECONCILE] Deposit ${orderNo} still pending on PalmPay (status=${remoteStatus})`
                        );
                    }
                } catch (err: any) {
                    logger.error(
                        `[RECONCILE] Failed to query PalmPay for orderNo=${orderNo}: ${err.message}`
                    );
                }
            }
        } catch (error: any) {
            logger.error('[RECONCILE] Reconciliation job error', error);
        }
    }
    ...
    async sendUserWebhook(userId: string, event: string, data: any): Promise<void> {
        try {
            ...
            if (!user.webhookUrl) {
                logger.debug(`[WEBHOOK] User ${userId} has no webhook URL. Skipping ${event}.`);
                return;
            }

            if (!user.webhookActive) {
                logger.debug(`[WEBHOOK] User ${userId} webhook disabled. Skipping ${event}.`);
                return;
            }

            logger.info(`[WEBHOOK] Sending ${event} to user ${userId} @ ${user.webhookUrl}`);
            ...
            const userWebhookSecret =
                process.env.USER_WEBHOOK_SECRET || this.webhookSecret;
            ...
            try {
                ...
                logger.info(`[WEBHOOK] ✅ Delivered to ${user.webhookUrl}`);
            } catch (webhookError: any) {
                logger.error(
                    `[WEBHOOK] ❌ Failed to deliver to ${user.webhookUrl}: ${webhookError.message}`
                );

                // Notify user by email about the failure
                const companyName =
                    (await SystemSetting.findOne())?.general?.companyName || 'VTStack';
                ...
                await emailService.sendEmail(
                    user.email,
                    'Action Required: Webhook Delivery Failed',
                    emailHtml
                );
            }
        } catch (error: any) {
            logger.error(`[WEBHOOK] Critical error in sendUserWebhook: ${error.message}`);
        }
    }
}
