...
import { authenticate, AuthenticatedRequest, generateToken, requireAdmin, requireSuperAdmin, auditMiddleware } from '../middleware';
...
const isAdmin = (req: AuthenticatedRequest, res: Response, next: any) => {
    if ((req as any).user && ((req as any).user.role === 'admin' || (req as any).user.role === 'subadmin')) {
        next();
    } else {
        res.status(403).json({
            success: false,
...
router.get('/admins', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const admins = await User.find({ role: { $in: ['admin', 'subadmin'] } }).select('-passwordHash').sort({ createdAt: -1 });
        res.json({
            success: true,
            data: admins,
        });
 ...
 */
router.post('/admins', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
 ...
 */
router.delete('/admins/:id', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
 ...
 */
router.patch('/tenants/:id/email', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        ...
        import('../services/EmailService').then(({ emailService }) => {
            emailService.sendAdminEmailChangeNotification(
                oldEmail,
                formattedEmail,
 ...
 */
router.post('/tenants/:id/adjust-wallet', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        ...
        const transaction = await walletService.manualAdjustment(
            id,
 ...
 */
router.get('/virtual-accounts/:id', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const virtualAccount = await VirtualAccount.findById(id).populate('userId', 'email firstName lastName businessName phone');
 ...
 */
router.patch('/virtual-accounts/:id/status', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const { status } = req.body;
        ...
        const virtualAccount = await VirtualAccount.findByIdAndUpdate(
            id,
            { status },
            { new: true }
        );
...
router.get('/transactions', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        if (virtualAccount) query['metadata.virtualAccount'] = virtualAccount;
 ...
 */
router.patch('/settings', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        let settings = await SystemSetting.findOne();
        if (!settings) {
