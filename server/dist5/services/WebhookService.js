    ...
    async sendUserWebhook(userId, event, data) {
        try {
            ...
            if (!user)
                return;
            if (!user.webhookUrl) {
                logger_1.logger.debug(`User ${userId} does not have a webhook URL configured. Skipping webhook for ${event}.`);
                return;
            }
            if (!user.webhookActive) {
                logger_1.logger.debug(`User ${userId} has webhook notifications disabled. Skipping webhook for ${event}.`);
                return;
            }
            ...
            try {
                ...
                logger_1.logger.error(`Failed to send user webhook to ${user.webhookUrl}: ${webhookError.message}`);
            ...
            logger_1.logger.error(`Critical error in sendUserWebhook: ${error.message}`);
        }
    }
}
