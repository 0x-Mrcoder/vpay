...
// Middleware to check if user is super admin for inline routes (if any)
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const statsAggregation = await models_1.Transaction.aggregate([
            ...
            },
            {
                $group: {
                    _id: null,
                    // Quarterly Stats
                    q1Inflow: { $sum: { $cond: [{ $and: [{ $lt: ["$createdAt", new Date(selectedYear, 3, 1)] }, { $eq: ["$type", "credit"] }, { $eq: ["$status", "success"] }] }, "$amount", 0] } },
                    q1Outflow: { $sum: { $cond: [{ $and: [{ $lt: ["$createdAt", new Date(selectedYear, 3, 1)] }, { $eq: ["$type", "debit"] }] }, "$amount", 0] } },
                    q2Inflow: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", new Date(selectedYear, 3, 1)] }, { $lt: ["$createdAt", new Date(selectedYear, 6, 1)] }, { $eq: ["$type", "credit"] }, { $eq: ["$status", "success"] }] }, "$amount", 0] } },
                    q2Outflow: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", new Date(selectedYear, 3, 1)] }, { $lt: ["$createdAt", new Date(selectedYear, 6, 1)] }, { $eq: ["$type", "debit"] }] }, "$amount", 0] } },
                    q3Inflow: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", new Date(selectedYear, 6, 1)] }, { $lt: ["$createdAt", new Date(selectedYear, 9, 1)] }, { $eq: ["$type", "credit"] }, { $eq: ["$status", "success"] }] }, "$amount", 0] } },
                    q3Outflow: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", new Date(selectedYear, 6, 1)] }, { $lt: ["$createdAt", new Date(selectedYear, 9, 1)] }, { $eq: ["$type", "debit"] }] }, "$amount", 0] } },
                    q4Inflow: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", new Date(selectedYear, 9, 1)] }, { $eq: ["$type", "credit"] }, { $eq: ["$status", "success"] }] }, "$amount", 0] } },
                    q4Outflow: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", new Date(selectedYear, 9, 1)] }, { $eq: ["$type", "debit"] }] }, "$amount", 0] } },
                    // Selected Month Stats
                    selectedMonthInflow: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", startOfMonth] }, { $lt: ["$createdAt", endOfMonth] }, { $eq: ["$type", "credit"] }, { $eq: ["$status", "success"] }] }, "$amount", 0] } },
                    selectedMonthOutflow: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", startOfMonth] }, { $lt: ["$createdAt", endOfMonth] }, { $eq: ["$type", "debit"] }] }, "$amount", 0] } },
                    // Daily Inflow (Only if current day is within selected year/month)
                    dailyInflow: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", today] }, { $eq: ["$type", "credit"] }, { $eq: ["$status", "success"] }] }, "$amount", 0] } },
                    dailyOutflow: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", today] }, { $eq: ["$type", "debit"] }] }, "$amount", 0] } },
                    // Yearly Stats
                    yearlyInflow: { $sum: { $cond: [{ $and: [{ $eq: ["$type", "credit"] }, { $eq: ["$status", "success"] }] }, "$amount", 0] } },
                    yearlyOutflow: { $sum: { $cond: [{ $eq: ["$type", "debit"] }, "$amount", 0] } },
                    // Fee Stats (Deducting 0.5% PalmPay fee to see real profit)
                    dailyFee: { $sum: { $cond: [{ $gte: ["$createdAt", today] }, { $subtract: ["$fee", { $multiply: ["$amount", 0.005] }] }, 0] } },
                    monthlyFee: { $sum: { $cond: [{ $and: [{ $gte: ["$createdAt", startOfMonth] }, { $lt: ["$createdAt", endOfMonth] }] }, { $subtract: ["$fee", { $multiply: ["$amount", 0.005] }] }, 0] } },
                    yearlyFee: { $sum: { $subtract: ["$fee", { $multiply: ["$amount", 0.005] }] } }
                }
            }
        ]);
        const tStats = statsAggregation[0] || {
            ...
            dailyFee: 0, monthlyFee: 0, yearlyFee: 0
        };
        ...
        const totalVirtualAccounts = await models_1.VirtualAccount.countDocuments();
        ...
        res.json({
            ...
            data: {
                transactions: {
                    ...
                    dailyFee: tStats.dailyFee || 0,
                    monthlyFee: tStats.monthlyFee || 0,
                    yearlyFee: tStats.yearlyFee || 0,
                    // Quarterly Stats
                    quarters: {
                ...
                tenants: {
                    ...
                    admins: totalAdmins,
                    virtualAccounts: totalVirtualAccounts,
                },
                webhooks: {
                    total: totalWebhooks,
...
router.get('/admins', async (req, res) => {
    try {
        const admins = await models_1.User.find({ role: { $in: ['admin', 'subadmin'] } }).select('-passwordHash').sort({ createdAt: -1 });
        res.json({
            success: true,
            data: admins,
        });
 ...
 */
router.post('/admins', middleware_1.requireSuperAdmin, async (req, res) => {
    try {
 ...
 */
router.delete('/admins/:id', middleware_1.requireSuperAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/tenants/:id', async (req, res) => {
    try {
        ...
        const accountsWithVolume = await Promise.all(virtualAccounts.map(async (acc) => {
            const stats = await models_1.Transaction.aggregate([
                {
                    $match: {
        ...
        res.json({
            ...
            data: {
                user,
                wallet,
                virtualAccounts: accountsWithVolume,
                stats: {
...
/**
 * Toggle tenant settlement
 * PATCH /api/admin/tenants/:id/settlement
 */
router.patch('/tenants/:id/settlement', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (typeof enabled !== 'boolean') {
            res.status(400).json({
                success: false,
                message: 'Invalid enabled status. Must be boolean.',
            });
            return;
        }
        const updatedUser = await models_1.User.findByIdAndUpdate(id, { settlementEnabled: enabled }, { new: true }).select('-passwordHash');
        if (!updatedUser) {
            res.status(404).json({
                success: false,
                message: 'Tenant not found',
        ...
        res.json({
            success: true,
            message: `Tenant settlement ${enabled ? 'enabled' : 'disabled'} successfully`,
            data: updatedUser,
        });
    }
    catch (error) {
        console.error('Toggle tenant settlement error:', error);
        res.status(500).json({
            success: false,
            message: 'Failed to toggle tenant settlement',
        });
    }
});
/**
 * Request Wallet Adjustment OTP
 * POST /api/admin/tenants/request-adjustment-otp
 */
router.post('/tenants/request-adjustment-otp', middleware_1.requireAdmin, async (req, res) => {
    try {
        const adminId = req.user?.id;
        const otp = Math.floor(100000 + Math.random() * 900000).toString();
        const expires = new Date(Date.now() + 5 * 60 * 1000); // 5 minutes
        await models_1.User.findByIdAndUpdate(adminId, {
            resetPasswordOtp: otp,
            resetPasswordOtpExpires: expires
        });
        await EmailService_1.emailService.sendWalletAdjustmentOtp(otp);
        res.json({
            success: true,
            message: 'Security OTP has been sent to the authorized email address.',
        });
    }
    catch (error) {
        console.error('Request Adjustment OTP error:', error);
        res.status(500).json({
            success: false,
            message: 'Failed to send security OTP',
        });
    }
});
/**
 ...
 */
router.post('/tenants/:id/adjust-wallet', middleware_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { amount, type, narration, securityPassword, securityOtp } = req.body;
        const adminId = req.user?.id;
        if (!amount || !type || !narration || !securityPassword || !securityOtp) {
            res.status(400).json({
                success: false,
                message: 'All security fields (Password, OTP) and adjustment details are required',
            });
            return;
        }
        // 1. Verify Security Password
        if (securityPassword !== '202526') {
            res.status(401).json({
                success: false,
        ...
        // 2. Verify OTP
        const adminUser = await models_1.User.findById(adminId);
        if (!adminUser || !adminUser.resetPasswordOtp || adminUser.resetPasswordOtp !== securityOtp) {
            res.status(401).json({
                success: false,
                message: 'Invalid security OTP',
            });
            return;
        }
        if (!adminUser.resetPasswordOtpExpires || adminUser.resetPasswordOtpExpires < new Date()) {
            res.status(401).json({
                success: false,
                message: 'Security OTP has expired',
            });
            return;
        }
        // Clear OTP after use
        ...
        if (!amount || !type || !narration) {
            res.status(400).json({
                success: false,
                message: 'Amount, type (credit/debit), and narration are required',
            });
            return;
        }
        ...
        const transaction = await WalletService_1.walletService.manualAdjustment(id, amount * 100, // Convert to kobo
        type, narration, req.user.id);
        res.json({
            success: true,
            message: `Wallet adjusted successfully: ${type} of ${amount}`,
            data: transaction
        });
    }
    catch (error) {
 ...
 * GET /api/admin/virtual-accounts/:id
 */
router.get('/virtual-accounts/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const virtualAccount = await models_1.VirtualAccount.findById(id).populate('userId', 'email firstName lastName businessName phone');
        if (!virtualAccount) {
            res.status(404).json({ success: false, message: 'Virtual account not found' });
            return;
        }
        res.json({ success: true, data: virtualAccount });
    }
    catch (error) {
...
/**
 * Update virtual account status (Freeze/Unfreeze)
 * PATCH /api/admin/virtual-accounts/:id/status
 */
router.patch('/virtual-accounts/:id/status', async (req, res) => {
    try {
        const { id } = req.params;
        const { status } = req.body;
        if (!['active', 'frozen'].includes(status)) {
            res.status(400).json({ success: false, message: 'Invalid status. Must be active or frozen' });
            return;
        }
        const virtualAccount = await models_1.VirtualAccount.findByIdAndUpdate(id, { status }, { new: true });
        if (!virtualAccount) {
            res.status(404).json({ success: false, message: 'Virtual account not found' });
            return;
        }
        res.json({ success: true, message: `Account ${status === 'frozen' ? 'frozen' : 'unfrozen'} successfully`, data: virtualAccount });
    }
    catch (error) {
        console.error('Update virtual account status error:', error);
        res.status(500).json({ success: false, message: 'Failed to update virtual account status' });
    }
});
/**
...
router.get('/transactions', async (req, res) => {
    try {
        const { limit = '50', offset = '0', type, status, category, tenantId, virtualAccount, search } = req.query;
        console.log('GET /transactions query:', { type, status, category, tenantId, virtualAccount, search });
        ...
        if (virtualAccount)
            query['metadata.virtualAccount'] = virtualAccount;
        ...
        if (wasFailed) {
            ...
            if (wallet) {
                if (transaction.type === 'debit') {
                    ...
                    // Now we are saying it succeeded, so we must deduct again.
 ...
 */
router.patch('/settings', middleware_1.requireSuperAdmin, async (req, res) => {
    try {
        let settings = await models_1.SystemSetting.findOne();
        if (!settings) {
