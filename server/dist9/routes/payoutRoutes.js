...
router.post('/', async (req, res) => {
    try {
        ...
        if (!amount || !bankCode || !accountNumber || !accountName || !pin) {
            res.status(400).json({
                success: false,
                message: 'Missing required fields (amount, bankCode, accountNumber, accountName, pin)',
            });
            return;
        }
        ...
        if (!user.transactionPinHash) {
            res.status(400).json({
                success: false,
                message: 'Transaction PIN is not set. Please set it in your security settings before withdrawing.',
            });
            return;
        }
